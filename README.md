...
Wrangle your [Windows Terminal](https://github.com/microsoft/terminal) tabs from a browser. `wt-wrangler` enumerates every tab across all open Windows Terminal windows and lets you **search** them by title, **summon** one to the foreground, or **close** it — from a dark, keyboard-friendly web UI on `localhost`. The list refreshes live, so changes you make elsewhere show up automatically.
